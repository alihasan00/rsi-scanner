...
import {
  defaultResearchExecution, RESEARCH_SAMPLE_CANDLES, runStrategyResearch, strategyResearchToCsv,
} from '../lib/strategyResearch'
import type { ResearchExecution, ResearchPartition, StrategyResearchReport } from '../lib/strategyResearch'
import './ResearchPanel.css'

export interface ResearchPanelProps { symbol: string; timeframe: Timeframe; market: ScreenerMarket }
type RunState =
  | { status: 'idle' }
  | { status: 'running'; progress: HistoryProgress | null; phase: string }
  | { status: 'done'; report: StrategyResearchReport }
...
function ResearchRun({ symbol, timeframe, market }: ResearchPanelProps) {
  ...
  return (
    <section className="research-panel" aria-label="Strategy research">
      <div className="research-panel__heading">
        <strong>Out-of-sample research</strong>
        <Tag>{MARKETS[market].venue} · {symbol} · {timeframe}</Tag>
      </div>
      <p>
        Compare Wilder RSI 12, 14 and 16 with two independently defined experiments: trend-weighted
        and adaptive RSI. Each recalculates the existing divergence lifecycle. The live scanner keeps
        Wilder RSI 14. Your RSI 50 study remains available in Divergences.
      </p>
      <p>
        ...
        trades are required; otherwise the baseline is retained. Use the parameter table to check
        neighboring periods, and collect unseen dates before choosing live rules.
      </p>
      <div className="research-panel__inputs">
        <label className="research-panel__field">
          <span>Sample after warmup</span>
          <Select
            aria-label="Research sample size" size="small" value={sampleCandles} disabled={running}
        ...
        <label className="research-panel__field">
          ...
          <input type="date" aria-label="Last UTC date" value={endDate} disabled={running} onChange={(event) => { setEndDate(event.target.value); setRun({ status: 'idle' }) }} />
        </label>
      ...
      <p className="research-panel__muted">
        Entry: next candle open after confirmation. Fixed ATR(14) stop/target, one position at a time,
        1× equity. {market === 'spot' ? 'Spot trades are long-only.' : 'Futures tests both directions with an assumed daily carrying cost.'}
        {' '}1 bp = 0.01%. Stop wins ambiguous same-bar touches. The full outcome horizon must remain inside its own period.
      </p>
      <Space wrap>
        {running
        ...
        {report && <>
          <Button size="small" icon={<DownloadOutlined />} onClick={() => download(`${market}_${symbol}_${timeframe}_research.csv`, strategyResearchToCsv(report), 'text/csv')}>Export CSV</Button>
          <Button size="small" icon={<DownloadOutlined />} onClick={() => download(`${market}_${symbol}_${timeframe}_research.json`, JSON.stringify(report, null, 2), 'application/json')}>Export full JSON</Button>
        </>}
      </Space>
      ...
      {report && selected && <>
        ...
        <div className="research-panel__scroll" tabIndex={0} role="region" aria-label="Selected candidate results by period">
          <table>
            ...
            <tbody>{partitions.map((partition) => {
              const { summary, window } = selected[partition]
              return <tr key={partition}>
                <th scope="row"><span className="research-panel__capitalize">{partition}</span><small>{date(window.startTime)} → {date(window.endTime)}</small><small>{window.candleCount.toLocaleString()} candles</small></th>
        ...
        <div className="research-panel__scroll" tabIndex={0} role="region" aria-label="Parameter sensitivity and holdout comparison">
          <table>
            <caption>Parameter sensitivity · all candidates, same dated splits</caption>
        ...
        <p className="research-panel__muted">Holdout exclusions: {selected.holdout.excluded.boundary} incomplete boundary horizons, {selected.holdout.excluded.gap} missing-data horizons, {selected.holdout.excluded.overlap} overlapping trades, {selected.holdout.excluded.spotShort} Spot shorts, {selected.holdout.excluded.atr} unavailable/invalid ATR, {selected.holdout.excluded.insolvent} after depleted equity.</p>
        <details>
          <summary>Methodology, oscillator definitions and recent holdout trades</summary>
          <p>{report.methodology}</p>
          <p>Trend-weighted RSI weights close changes that agree with the trailing net change by 1 + efficiency. Efficiency is absolute net change divided by total absolute changes over 14 bars. Adaptive RSI uses Wilder smoothing with an effective period of 14 × (1.5 − efficiency). Both are independently defined experiments; they are not reproductions of LuxAlgo indicators.</p>
