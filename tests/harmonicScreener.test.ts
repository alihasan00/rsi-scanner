...
describe('harmonic closed-history cache', () => {
  ...
  test('same-length anchor corrections, gaps, and resets invalidate cached setups', () => {
    ...
    expect(getHarmonicAnalysis('HARMONIC-CORRECTION', gap)).toMatchObject({ setups: [], closedBarCount: 7, historyIssue: 'gap', continuity: { state: 'reset' } })
    expect(getHarmonicAnalysis('HARMONIC-CORRECTION', [])).toEqual({ setups: [], closedBarCount: 0 })
  })

  test('older correction evidence participates, and market/timeframe keys stay isolated', () => {
    ...
    const corrected = getHarmonicAnalysis('spot:1h:HARMONIC-BOUND', oldCorrection)
    ...
    expect(getHarmonicAnalysis('spot:1h:HARMONIC-BOUND', long).setups).toEqual([])
  })
})
